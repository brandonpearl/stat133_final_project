...
fluidPage(
  ...
  sidebarPanel(
    selectInput('x_var', 'X Variable', names(merge_data)[c(2:9, 11:13, 17, 18, 21, 23:29, 36:43)], selected = names(merge_data)[13]),
    selectInput('y_var', 'Y Variable', names(merge_data)[c(2:9, 11:13, 17, 18, 21, 23:29, 36:43)], selected = names(merge_data)[13]),
    radioButtons("color_by", label = "Color", 
                 choices = c("Team" = "team", "Position" = "position"),
                 selected = "team"),
    checkboxInput("regression_by", label = "Show Regression Line", value = FALSE),
