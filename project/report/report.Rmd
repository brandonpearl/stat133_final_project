...
As anticipated, EFF and salary has a positive correlation (r = 0.474). This is reasonable as managers of each team will be willing to recruit players based on their over all skill levels. However, therefore there are several factors that weakened the correlation.
...
Also, in reality, the managers not only look at player's data while drafting. They watch how they play and look at things that cannot be recorded.
In addition, salary is a relative measure not an absolute measure. In other words, there is no set salary performming one mroe steal or shooting a point. Rather, managers usually evaluate a player's data and the actual performance at a game and offer somewhat arbitrary rounded number of salary determined by comparing with other players in the team or in the league.  

There were several interesting results regarding the correlation between variables. 
First, experience showed relatively weak correlation with EFF (r = 0.209) which demonstrated that the years a player played in NBA has not much relation with his overall skill level. We suspect there are two major trends happening together. First, as the positive correlation shows, more experience at the League helps a player to perform better. This can be because players get used to the atmosphere and and the pressure while playing at the premiere League. Second, some rookie players perform better than the original players as they feel obliged to stand out. Rookie players think playing at the NBA is a great chance for them that they do not want to let go this chance.

It was also interesting to see how EFF was weakly correlated with points while strongly correlated with steals (r = 0.605) and defensive rebounds (r = 0.698, r = 0.706). This has to do with the positions that players play. While point guards have a lot of chance to shoot a goal, other positions have relatively few chances to do so. As the correlation takes all positions into account, points have a relatively small correlation with the efficiency level. Meanwhile, steals and rebounds are moves that all players perform. Therefore, it is strongly correlated with the EFF.

There were several notable results from eda. First, majority of players did not have a data for college. While it is possible that some are missing values, considering that all other variables rarely had a missing data, it is reasonable to assume that there are some other reason. Basketball Reference includes school information as long as a player has ever been matriculated. This means that even if a player drops out or graduates, Basketball Refernce will continue recordin that data. This means that most players did not go to college from the beginning.

Conclusions: Summarize the main results of the analysis. Who could benefit from your
analysis? Based on your results, what other questions and/or hypotheses can you come up
with? What other future analyses would you like to perform (time permitting)? Make relevant
conclusions explicitly connected to the analysis and context. 


# CONCLUSIONS

In the future, I would plot separate plots for each postion and let Shinyapp compute correlation coefficient of two variables for each of the position. By comparing r value for each position, it will help setting standards for each position during drafting. 
Also, we are interested in the correlation between the number of wins and salary. While individual performance level matters, basketball is about the team. If a player brings more wins regardless of his recorded performance, there is something about the player that brings success to the game. 
If possible, it would be also interesting to participate in the data collection process. While the current measure of performances are valid. We feel there are more measures that can be incorporated to make eff more reliable and valid which is tightly connected to the wins that a player bring to a team. By making the eff more exact, managers do not have to evaluate players with the values that aren't recorded.   
